...
/*
tbl_score 데이터는 한 학생의 과목점수가 각각 레코드 별로 저장되어 있다.
과목수가 늘어나면 학번,과목명,점수형식으로 계속 저장해주면 된다.
...
필요에 따라 Super Key로PK 를 설정하기도 하지만 Super Key는 잘못 사용하면 데이터 
무결성을 해칠 수 있다

UPDATE tbl_student SET sc_score =90
WHER st_num ='111'AND sc_subject ='국어'
...
-- 학번 S0044인 미술 점수를 50점에서 80점으로 바꾸고 싶다.
--UPDATE 나 DELET를 실행할때 조건절에 복수의 칼럼 조건을 부여하는것은 썩 좋지않다.
...
--2.변경하고자하는 데이터가 조회되면 PK가 무엇인지 확인한다
--PK :308

--PK 를 조건으로 하는 UPDATE 명령을 실행한다
UPDATE tbl_score SET sc_score=80
WHERE sc_seq=308;

--4.실행후에는 정확히 원하는 값으로 변경되었는지 확인
...
--성적 조회를 하였는데 학번과 점수만 나타나서 누구인지(학생이름)을 알수 가없다
--두개이상의 테이블을 서로 연동하여 데이터를 확인하기
--JOIN 하여 데이터 검색

--1.tbl_score table 데이터를 전부 펼치고
--2.sc_stnum 칼럼과 sc_score 칼럼 데이터를 보여주면서
--3.tbl_score.sc_stnum 데이터를 가지고 tbl_student.st_num 를 SELECT 한후
