...
-- 가장 먼저 어떤 DB를 사용할 것인지 알려줘야한다.
USE mydb;
drop table tbl_test;

-- table을 생성할때 칼럼의 이름은  사전에 있는 키워드는 가급적 사용하지 않는것이 좋다.
-- 사전에 있는 키워드를 사용할때는 접두사를 부착하여 사용하는 것이 좋다.
...
/*
DBMS 중에서 가장많이 사용하는 오라클과 함께 Mysql을 사용하는데 
mysql 에서 사용하는 방식이 dbms 의 업계 표준으로 정착이 된 상태alter

대부분의 DBMS들은 먼저 Database를 생성하고 
사용자가 Database에 접근할수 있ㅎ는 권한을 부여하여
사용자는 Database를 사용하겠다 라고 선언을 한후
DB 관련된 명령을 사용한다.
*/
...
-- 다중 데이터 INSERT 하기
INSERT INTO tbl_test (t_name)
values('임지선'),('임모찌'),('임모카'),('임민섭');

SELECT * FROM tbl_test;

/*
tbl__test 의 ID칼럼은 AUTO_INCREMENT 로 설정되었기 때문에
데이터를 INSERT 할때마다 자동으로SEQUNCE 값이 생성되어 추가된다

5.X MYSQL에서는 AUTO_INCREMENT 칼럼에 이슈가 있었다
5.X 숫자 TYPE은 주로 INT 형을 사용하는데 INT형은 정수 11자리이다.
때문에 정수형 11자리 범위를 벗어나는 경우 데이터 INSERT가 불가능해지는 문제가 있었다.
5.7버전 이후에는 BIGINT 형 타입이 도입되어 문제를 해결하였다.
DB TABLE을 만들때 AUTO_INCREAMENT 칼럼에는 반드시 BIGINT 형을 지정한자alter

MYSQL숫자형 데이터 타입
TINYINT :1byte :-128~127까지만 저장가능
SMALLINT:2byte :-32768~23762까지
MEDIUMINT:3Byte :0~16777215,-8388608~8388607까지
INT :4Byte :-217483648~214783647
BIGINT : 8Byte :-2의7승 -1 부터 2의 7승까지
 

*/
/*
AUTO_INCREMENT 칼럼을 만들때 주의할 점 데이터 타입은 BIGINT 로 설정
가급적 PRIMARY KEY로 설정할것
PK가 아닐경우는 반드시 NOT NULL 과 UNIQUE를 같이 설정한다
PK또는 NOT NULL and UNIQUE 설정이 되지 않았을 경우
TABLE 생성에서 오류가 난다
*/
...
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('s0101','임효선','중국어',4,'7777','ㅇㅇㅇ');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('s0102','임효선','중국어',4,'7777777','ㅇㅇㅇ');
Insert into TBL_STUDENT (ST_NUM,ST_NAME,ST_DEPT,ST_GRADE,ST_TEL,ST_ADDR) values ('S0101','임효선','중국어',4,'01032349007','화정동');
